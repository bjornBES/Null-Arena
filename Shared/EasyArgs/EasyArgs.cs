...
namespace Shared.EasyArgs
{
    ...
    public static class EasyArgs
    {
        ...
        public static object Parse(Type type, string[] rawArgs)
        {
            ...
            string[] args = NormalizeRawArgs(rawArgs);
            ...
            Dictionary<string, PropertyInfo> optionMap = new Dictionary<string, PropertyInfo>(StringComparer.Ordinal);
            Dictionary<char, PropertyInfo> shortMap = new Dictionary<char, PropertyInfo>();
            Dictionary<PropertyInfo, ArgAttribute> propertyMeta = new Dictionary<PropertyInfo, ArgAttribute>();
            SortedDictionary<int, PropertyInfo> positionalProps = new SortedDictionary<int, PropertyInfo>();
            ...
            List<string> consumedPositional = new List<string>();
            int i = 0;
            ...
            foreach (var kv in positionalProps)
            {
                ...
                {
                    PositionalAttribute? positionalAttribute = prop.GetCustomAttribute<PositionalAttribute>();
                    if (positionalAttribute != null && positionalAttribute.Required)
                    {
        ...
        static void SetProperty(object target, PropertyInfo prop, string rawValue)
        {
            ...
            if (propType.IsArray)
            {
                ...
                string[] parts = SplitArrayValues(rawValue);
                Array arr = Array.CreateInstance(elemType, parts.Length);
            ...
            if (propType.IsEnum)
            {
                object val = Enum.Parse(propType, rawValue, ignoreCase: true);
            ...
            if (underlying != null)
            {
                ...
                { prop.SetValue(target, null); return; }
                object converted = ConvertTo(rawValue, underlying);
            ...
            if (IsBoolean(propType))
            {
                ...
                bool v = ParseBoolString(rawValue);
                prop.SetValue(target, v);
                return;
            }

            // numeric and other
            object convertedVal = ConvertTo(rawValue, propType);
        ...
        static string GenerateHelp(Type t)
        {
            ...
            // positional
            List<(int, PropertyInfo, PositionalAttribute)> positional = new List<(int, PropertyInfo, PositionalAttribute)>();
            foreach (var p in t.GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
        ...
        {
            SortedDictionary<int, PropertyInfo> positional = new SortedDictionary<int, PropertyInfo>();
            foreach (var p in t.GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                var pa = p.GetCustomAttribute<PositionalAttribute>();
                if (pa != null)
